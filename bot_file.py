...
from observer_pattern.observer.db_observer.db_obs import DB_Observer
from observer_pattern.db_start import db_start
from observer_pattern.observer.sender_observer.timetable import Timetable_maker
...
def register_handlers(dp: Dispatcher, controller : Controller, db_obs : DB_Observer):
    ...
    dp.register_message_handler(lambda message: on_stop_command(message, db_obs), commands=['stop'])
    dp.register_message_handler(lambda message: on_edit_command(message, db_obs), commands=['edit'])
    ...
    dp.register_callback_query_handler(lambda call: change_reminder(call, controller, db_obs))
